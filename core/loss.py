        ...
        with torch.no_grad():
            ...
            """if features is not None:
                ...
                for scale, feature in enumerate(features):
                    if self.disentangled == 0:
                        scale_out_prob = outputs["pred_logits"].sigmoid()  # [batch_size, num_queries, num_classes]
                    else:
                        scale_out_prob = torch.softmax(outputs['pred_logits'], dim=-1) * outputs['pred_objectness']
    
                    scale_out_bbox = outputs["pred_boxes"]  # [batch_size, num_queries, 4]
    
                    scale_indices = []
                    scale_matched_ids = []

                    unknown_labels, ow_indices = [], []
                    for batch_idx in range(bs):
                        bz_boxes = scale_out_bbox[batch_idx]  # [num_proposals, 4]
                        bz_out_prob = scale_out_prob[batch_idx]
                        bz_tgt_ids = targets[batch_idx]["labels"]
                        num_insts = len(bz_tgt_ids)
                        if num_insts == 0:  # no objects in this image
                            ...
                            scale_indices.append(indices_batchi)
                            scale_matched_ids.append(matched_qidx)
                        else:
                    ...
                    scale_costs.append((scale_indices, scale_matched_ids))

                indices = []
                matched_ids = []
                for scale_indices, scale_matched_ids in scale_costs:
                    indices.extend(scale_indices)
                    matched_ids.extend(scale_matched_ids)


            else:
                # Default single-scale processing if multi-scale features are not provided
                indices, matched_ids, unknown_labels, ow_indices = [], [], [], []

                for batch_idx in range(bs):
                    bz_boxes = outputs["pred_boxes"][batch_idx]  # [num_proposals, 4]
                    bz_out_prob = outputs["pred_logits"].sigmoid()[batch_idx]  # [num_proposals, num_classes]
                    bz_tgt_ids = targets[batch_idx]["labels"]
                    num_insts = len(bz_tgt_ids)
                    if num_insts == 0:  # no objects in this image
                        non_valid = torch.zeros(bz_out_prob.shape[0]).to(bz_out_prob) > 0
